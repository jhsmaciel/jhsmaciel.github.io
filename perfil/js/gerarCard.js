...
let contador = 0
function makeCarousel(link, src) {
    let $carousel = document.createElement('div')
        $carousel.className = contador ?  "carousel-item" : "carousel-item active"
    let $link = document.createElement('a')
        $link.href = link
    let $img = document.createElement('img')
        $img.src = src
        $img.className = "img-fluid widthZ"
    $carousel.appendChild($link);
    $link.appendChild($img);
    document.querySelector('#carousel-top').appendChild($carousel);
    contador++
}